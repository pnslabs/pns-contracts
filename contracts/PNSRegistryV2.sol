...
contract PNSRegistryV2 is Initializable, AccessControlUpgradeable, IPNSSchema {
	/// Expiry time value
	...
	function _setPhoneRecord(
		...
		address resolver,
		string memory label
	) internal onlyVerifiedOwner(phoneHash) {
	...
	modifier onlyVerifiedOwner(bytes32 phoneHash) virtual {
		PhoneRecord memory recordData = records[phoneHash];
		require(recordData.isVerified && recordData.owner == msg.sender, 'caller is not verified owner');
		_;
	}
